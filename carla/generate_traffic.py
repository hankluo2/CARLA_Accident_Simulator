...
def exp_dir(prfx='_out', save_dir='D:\\DATASET\\ExperimentData\\New') -> str:
...
def get_vehicle_actor_in_roi(world, vehicle_list, regions):
    for vid in vehicle_list:
        vehicle = world.get_actor(vid)
        y, x = vehicle.get_location().x, vehicle.get_location().y  # invert x & y axis in practice
...
def launch():
    ...
    try:
        ...
        # Camera of Scene 2
        camera_transform = carla.Transform(carla.Location(x=-35, y=7, z=6), carla.Rotation(yaw=130))
        ...
        # Scene 2
        regionX1, regionX2 = [-37.9, -41.0, -53.3,
                              -56.6], [-37.9, -56.6, -69.1, -94.4, -91.6, -73.1, -60.4, -47.7, -40.5, -37.9]
        regionY1, regionY2 = [29.6, 66.8, 59.4, 30.0], [29.6, 29.9, 29.8, 29.7, 28.0, 17.1, 14.0, 10.1, 14.5, 18.3]
        ...
        # test: set every vehicle as danger vehicle
        # danger_vehilce = get_vehicle_actor_in_roi(world, vehicles_list, regions)
        # print("set {} as danger vehicle:".format(danger_vehilce.id))

        # TODO: set aggressive vehicle
        # Comment it first.
        for v in vehicles_list:
            ...
            traffic_manager.vehicle_percentage_speed_difference(danger_vehicle, -200)
        ...
        while True:
            if not args.asynch and synchronous_master:
                ...
                print('\nget vehicle locations')
                sensor_cnt = 0
                sim_length = 1  # minutes
                for _ in range(int(sim_length * 60 / tick_gap / args.number_of_vehicles)):
                    for _vehicle in vehicles_list:
                        ...
                        if location_in_region_of_interest(x, y, regions):
                            ...
                            print("Vehicle velocity:", this_vehicle.get_velocity())
                            print("Angular velocity:", this_vehicle.get_angular_velocity())
                            print("Vehicle acceleration:", this_vehicle.get_acceleration())
                            # ----------------
                            # imu sensor:
                            # ----------------
                            ...
                            time.sleep(tick_gap * 10)  # wait for sync
                        else:
                        ...
                        time.sleep(tick_gap * 10)  # TODO: modified
                try:
                    # for _ in range():
