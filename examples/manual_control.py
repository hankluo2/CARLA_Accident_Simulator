...
class World(object):
    ...
    def destroy(self):
        ...
        sensors = [
            ...
            self.lane_invasion_sensor.sensor, self.gnss_sensor.sensor, self.imu_sensor.sensor
        ]
    ...
    def tick(self, world, clock):
        ...
        if isinstance(c, carla.VehicleControl):
            self._info_text += [('Throttle:', c.throttle, 0.0, 1.0),
                                ('Steer:', c.steer, -1.0, 1.0), ('Brake:', c.brake, 0.0, 1.0),
                                ('Reverse:', c.reverse), ('Hand brake:', c.hand_brake),
...
class CameraManager(object):
            ...
            self._camera_transforms = [
                ...
                (carla.Transform(
                    carla.Location(x=+1.9 * bound_x, y=+1.0 * bound_y,
                                   z=1.2 * bound_z)), Attachment.SpringArm),
                (carla.Transform(
                    carla.Location(x=-2.8 * bound_x, y=+0.0 * bound_y, z=4.6 * bound_z),
                    carla.Rotation(pitch=6.0)), Attachment.SpringArm),
                (carla.Transform(carla.Location(x=-1.0, y=-1.0 * bound_y,
                                                z=0.4 * bound_z)), Attachment.Rigid)
            ]
        else:
            self._camera_transforms = [
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            dvs_events = np.frombuffer(image.raw_data,
                                       dtype=np.dtype([('x', np.uint16), ('y', np.uint16),
                                                       ('t', np.int64), ('pol', np.bool)]))
